...
@section('content')
<main id="main" class="vote-page" data-vote-root
    data-vote-url="{{ route('vote.store') }}"
    data-csrf="{{ csrf_token() }}"
    data-open="{{ $isOpen ? '1' : '0' }}"
    data-voted="{{ $votedEntryId ?: '' }}"
    ...
    <div class="vote-stats" role="region" aria-label="إحصاءات التصويت">
        ...
        <div class="vote-stat">
            ...
            <span class="vote-stat-label">إصدارات العام الماضي</span>
        </div>
        <div class="vote-stat-rule" aria-hidden="true"></div>
        <div class="vote-stat">
    ...
    @if ($leader)
        <section class="vote-leader" aria-labelledby="vote-leader-title">
            <div class="vote-leader-media">
                <img src="{{ $leader->image_url ?: $fallbackCover }}" alt="{{ $leader->title }} — {{ $leader->artist }}" width="720" height="720">
            ...
            <div class="vote-leader-body">
                ...
                @endif
                <div class="vote-bar" role="img" aria-label="نسبة الأصوات {{ $leader->shareOf($totalVotes) }} بالمئة">
                    <div class="vote-bar-fill" data-bar="{{ $leader->id }}" style="width: {{ $leader->shareOf($totalVotes) }}%"></div>
                </div>
    ...
    <ol class="vote-list">
        @foreach ($entries as $index => $entry)
            ...
            <li class="vote-card @if ($isVoted) is-voted @endif @if ($votedEntryId && ! $isVoted) is-locked @endif"
                ...
                style="--i: {{ $index }}">
                <span class="vote-card-num" data-rank="{{ $entry->id }}" aria-hidden="true">{{ str_pad((string) $rank, 2, '0', STR_PAD_LEFT) }}</span>
                <div class="vote-card-cover">
                    <img src="{{ $cover }}" alt="" width="160" height="160" loading="{{ $index < 2 ? 'eager' : 'lazy' }}">
                </div>
                <div class="vote-card-body">
                    ...
                    <div class="vote-bar" role="img" aria-label="نسبة الأصوات {{ $share }} بالمئة">
                        <div class="vote-bar-fill" data-bar="{{ $entry->id }}" style="width: {{ $share }}%"></div>
                    </div>
                    <div class="vote-card-meta">
                        <span><strong data-votes="{{ $entry->id }}">{{ number_format($entry->votes_count) }}</strong> صوت</span>
                ...
                <div class="vote-card-action">
                    <form method="POST" action="{{ route('vote.store') }}" data-vote-form>
                        ...
                        <button type="submit"
                            class="vote-btn"
                            data-vote-btn="{{ $entry->id }}"
                            @disabled(! $isOpen || $votedEntryId)
    ...
    <aside class="vote-rules" aria-labelledby="vote-rules-title">
        ...
        <ul>
            ...
            <li>القائمة من أبرز أغاني 2025 حسب منصات البث والنقد الموسيقي.</li>
