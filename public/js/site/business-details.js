...
function getId(){
  ...
  const m = location.pathname.match(/\/business\/(\d+)/);
  if (m) return m[1];
  ...
  return `
    <a href="/business/${encodeURIComponent(d.id)}" class="doc-list-card">
      ...
      <div class="doc-list-body">
        <div class="doc-list-spec">${esc(d.company || d.role || '')}</div>
...
async function loadRelated(currentId){
  try{
    const res = await fetch('/api/people?category=business&limit=12');
    const j = await res.json();
    const items = (j.data || []).filter(d => d.id !== currentId).slice(0,4);
    if (!items.length) return '';
    return `
      <section class="related-section">
        <h2 class="related-title">رجال أعمال <em>آخرون</em></h2>
...
async function init(){
  ...
  if (!id){
    main.innerHTML = `<div class="notfound"><h1>الملف غير موجود</h1><p>لم نتمكن من العثور على الملف المطلوب.</p><a href="/business">عرض جميع رجال الأعمال</a></div>`;
    return;
  }
  try{
    ...
    const j = await res.json();
    const d = j.data;
    if (!d || d.category !== 'business') throw new Error('wrong category');
    ...
    main.innerHTML = `
      <section class="profile-hero">
        ...
        <div class="profile-hero-grid">
          ...
          <div class="profile-info">
            <div class="profile-eyebrow">Business Profile · ملف أعمال</div>
            <div class="profile-spec">${esc(d.company || d.role || 'ريادة أعمال')}</div>
            ...
            <div class="profile-meta">
              ${d.company ? `<span>🏢 <b>${esc(d.company)}</b></span>` : ''}
      ...
      ${(d.stat || d.net_worth || d.company || d.badge) ? `
        <section class="profile-stats">
          <div class="profile-stats-inner">
            ${d.stat ? `<div class="pstat"><div class="pstat-num">${esc(d.stat)}</div><div class="pstat-lbl">${esc(d.stat_label || 'إنجاز')}</div></div>` : ''}
            ${d.net_worth ? `<div class="pstat"><div class="pstat-num small">${esc(d.net_worth)}</div><div class="pstat-lbl">التقييم / الثروة</div></div>` : ''}
            ${d.company ? `<div class="pstat"><div class="pstat-num small">${esc(d.company)}</div><div class="pstat-lbl">الشركة</div></div>` : ''}
    ...
    console.error(e);
    main.innerHTML = `<div class="notfound"><h1>الملف غير موجود</h1><p>لم نتمكن من العثور على رجل الأعمال المطلوب.</p><a href="/business">عرض جميع رجال الأعمال</a></div>`;
  }
}
